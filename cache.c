...
void allocate_blocks(struct cache* l1_data, struct cache* l1_inst, struct cache* l2){
     printf("allocating\n\r");     uint i = 0;     //The cache has to have enough space to store the pointers to each set (which is a number of blocks) 

     l1_data->cache_set = (struct cache_set*) malloc(l1_data->num_sets* sizeof(struct cache_block*));
     printf("allocating1\n\r");     //the set must have a pointer to each block     
	...
	printf("allocating2\n\r");     l1_inst->cache_set = (struct cache_set*)malloc(l1_inst->num_sets * sizeof(struct cache_block*));     
